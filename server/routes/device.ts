 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Firmware-facing device provisioning endpoint.
 *
 * No JWT required — authentication is via factoryKey in the request body.
 * This route is called by the Master Box ESP32 on first boot (or after
 * factory reset) to obtain its pondId + deviceToken from the cloud.
 *
 * Mounted at: /api/device
 *
 * POST /api/device/provision
 *   Body: { boxId, deviceType, factoryKey }
 *   Response 200: { success, pondId, deviceToken }
 */
...
//  POST /api/device/provision
//
//  Called by the Master Box firmware on first boot when NVS has no token.
//
//  Request body (JSON):
//    {
//      "boxId":      "MB001",
...
//
//  The firmware saves pondId + deviceToken to NVS via saveProvisionData().
...
router.post('/provision', async (req, res) => {
  try {
    ...
    res.json({
      ...
      deviceToken: device.apiKey,   // firmware reads as doc["deviceToken"]
      boxId:       device.boxId,
    });
