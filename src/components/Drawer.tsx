      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-y-0 left-0 w-[82%] max-w-[300px] bg-card z-[70] shadow-2xl flex flex-col"
          >
            {/* ── Header ── */}
            <div className="flex items-center justify-between px-5 pt-8 pb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-ink font-black text-lg tracking-tighter">AquaGrow</h2>
              </div>
              <button
                onClick={onClose}
            ...
            {/* ── User Profile Pill ── */}
            <div className="mx-5 mb-4 bg-[#F8F9FE] rounded-2xl px-4 py-3 flex items-center gap-3 border border-card-border">
              <div className="w-10 h-10 rounded-xl overflow-hidden border border-card-border flex-shrink-0">
                <img
              ...
              <div className="min-w-0">
                <p className="font-black text-sm tracking-tight text-ink truncate">{user.name}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={cn(
                    ...
                    'bg-[#4A2C2A]/5 border-[#4A2C2A]/10 text-ink/40'
                  )}>
                  ...
                  <span className="text-[7px] font-black text-ink/25 uppercase tracking-widest">{user.role}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-5">
              {sections.map((section, si) => (
                <div key={si}>
                  <p className="text-[8px] font-black text-ink/25 uppercase tracking-[0.2em] px-2 mb-2">
                    {section.title}
                  </p>
                        ...
                        >
                          <div className={cn(
                            ...
                            isActive
                              ...
                              : 'bg-[#4A2C2A]/5 text-ink/50 group-hover:bg-[#4A2C2A]/10'
                          )}>
                            <item.icon size={16} />
                          </div>
                          <span className={cn(
                          ...
                          {isActive && (
                            <div className="ml-auto w-1.5 h-1.5 bg-card rounded-full" />
                          )}
                        </button>
                      );
                    })}
            ...
            {/* ── Footer ── */}
            <div className="px-4 pb-8 pt-3 border-t border-card-border space-y-2">
