  ...
  return (
    <div className="pb-32 bg-[#FBFBFE] min-h-screen relative overflow-hidden text-left font-sans">
      ...
      >
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4">
                <div className={cn(
                  ...
                  isPro ? "bg-emerald-500/20 text-[#C78200]" : "bg-white/10 text-white/40"
                )}>
              ...
              <div className="flex items-center gap-3 bg-white/5 backdrop-blur-md rounded-xl p-3 border border-white/5 inline-flex">
        ...
        <motion.section variants={itemVariants} className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[#4A2C2A] text-base font-black tracking-tighter">{isPro ? 'Your Benefits' : 'Upgrade Benefits'}</h3>
            <div className="px-2 py-0.5 bg-[#4A2C2A]/5 rounded-full text-[7px] font-black uppercase tracking-widest text-[#4A2C2A]/40">
              {isPro ? 'Active' : 'Locked'}
            </div>
          </div>
          
          <div className="bg-white rounded-[1.5rem] p-4 shadow-sm border border-black/5 space-y-4">
            {proFeatures.map((f, i) => (
              <div key={i} className="flex items-center gap-3 group">
                <div className={cn(
                  "w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-sm border border-black/5 bg-paper",
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <p className={cn(
                      ...
                      isPro ? "text-[#4A2C2A]" : "text-[#4A2C2A]/30"
                    )}>{f.label}</p>
        ...
        <motion.div variants={itemVariants} className="pt-4 border-t border-black/5">
           <div className="bg-[#4A2C2A]/5 rounded-[1.5rem] p-5 flex items-center justify-between group">
              <div className="flex items-center gap-4">
                 <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-[#4A2C2A]/40 shadow-sm">
                    <Headphones size={20} />
                 </div>
                 <div>
                    <h4 className="text-[11px] font-black tracking-tight text-[#4A2C2A]">{t.paymentGlitch}</h4>
                    <p className="text-[8px] font-bold text-[#4A2C2A]/40 uppercase tracking-widest">{t.refundDetails}</p>
                 </div>
              </div>
              <button 
                onClick={() => window.open('tel:9123456789')}
                className="px-4 py-2 bg-white rounded-xl text-[9px] font-black uppercase tracking-widest text-[#4A2C2A] shadow-sm active:scale-95 transition-all hover:bg-[#4A2C2A] hover:text-white"
              >
                 {t.supportContact}
              </button>
           </div>
           ...
           <div className="flex items-center justify-center gap-6 mt-6">
              <button 
                ...
                className="text-[8px] font-black uppercase tracking-[0.2em] text-[#4A2C2A]/30 hover:text-[#C78200] transition-colors"
              >
                 {t.refundPolicy}
              </button>
        ...
        <motion.div variants={itemVariants} className="pt-2">
          {!isPro ? (
            ...
            <button 
              onClick={() => navigate('/subscription')}
              className="w-full bg-white text-[#4A2C2A] py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] border border-black/10 hover:border-[#C78200]/30 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              Manage Plan
           ...
           <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 overflow-hidden">
              ...
              >
                 ...
                 <div className="text-center mb-10">
                    <h2 className="text-3xl font-black tracking-tighter text-[#4A2C2A]">{selectedTransaction.amount}</h2>
                 ...
                 <div className="space-y-4 p-6 bg-[#4A2C2A]/5 rounded-[2rem] border border-[#4A2C2A]/5">
                    ...
                    <div className="flex justify-between items-center">
                       <p className="text-[8px] font-black uppercase tracking-widest text-[#4A2C2A]/30">{t.transactionId}</p>
                       <p className="font-black text-[10px] text-[#4A2C2A]/60 font-mono">{selectedTransaction.id}</p>
                    </div>
                    <div className="flex justify-between items-center">
                       <p className="text-[8px] font-black uppercase tracking-widest text-[#4A2C2A]/30">Status</p>
        ...
        {policyType && (
           <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-6 overflow-hidden">
              ...
              <motion.div 
                ...
                animate={{ y: 0 }}
                exit={{ y: "100%" }}
                className="relative w-full max-w-md bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 overflow-hidden max-h-[80vh] flex flex-col"
              >
                 <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-black tracking-tighter text-[#4A2C2A]">
                       {policyType === 'refund' ? t.refundPolicy : 'Terms & Conditions'}
                    </h3>
                    <button 
                      onClick={() => setPolicyType(null)}
                      className="p-3 bg-[#4A2C2A]/5 rounded-2xl text-[#4A2C2A]/30 hover:text-[#4A2C2A]"
                    >
                       <X size={20} />
                    </button>
                 </div>
                 ...
                 <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    {policyType === 'refund' ? (
                       ...
                       <div className="space-y-6 text-[#4A2C2A]/70 text-[11px] leading-relaxed font-medium">
                          <p>
                             <strong className="text-[#4A2C2A] block mb-1">1. SERVICE ACCESS</strong>
                          ...
                          <p>
                             <strong className="text-[#4A2C2A] block mb-1">2. FAIR USAGE</strong>
                          ...
                          <p>
                             <strong className="text-[#4A2C2A] block mb-1">3. DATA RESPONSIBILITY</strong>
                          ...
                          <p>
                             <strong className="text-[#4A2C2A] block mb-1">4. INTELLECTUAL PROPERTY</strong>
