      ...
      <div className="pt-24 px-8 py-8">
        ...
        <div className="space-y-8">
          {mockInventory.map((item, i) => (
            <div key={item.id} className="bg-card p-6 rounded-[2.5rem] shadow-sm flex items-center gap-6 group hover:border-[#C78200]/30 transition-all relative overflow-hidden border border-card-border">
              <div className={cn(
              ...
              <div className="flex-1">
                <p className="font-black text-lg text-ink tracking-tight">{item.name}</p>
                <div className="flex items-center gap-6 mt-2">
                  <p className="text-ink/40 text-[11px] font-black uppercase tracking-widest">₹{item.price}</p>
                  <p className="text-ink/40 text-[11px] font-black uppercase tracking-widest">{t.stock}: <span className="text-ink">{item.stock}</span></p>
                </div>
              </div>
              <button className="p-4 rounded-2xl bg-[#F8F9FE] text-ink/20 hover:text-[#C78200] hover:bg-card transition-all shadow-sm border border-card-border">
                <Settings size={20} />
              </button>
            </div>
          ))}
