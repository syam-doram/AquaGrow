...
export const DailyExpenseLog = ({ t }: { t: Translations }) => {
  ...
  const handleSave = () => {
    ...
    const existing = JSON.parse(localStorage.getItem('daily_expenses') || '[]');
    existing.push(newEntry);
    localStorage.setItem('daily_expenses', JSON.stringify(existing));
    
    setTimeout(() => {
      navigate('/expense-report'); // Return to expense report analytics
    }, 2500);
  };
