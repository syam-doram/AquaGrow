...
export const SmartFarmHub = ({ t }: { t: Translations }) => {
  ...
  const [activeTab, setActiveTab] = useState<'aerators' | 'electricity' | 'power'>('aerators');
...
{
  activeTab === 'aerators' && (
    <motion.div key="aerators" initial = {{ opacity: 0, y: 10 }
} animate = {{ opacity: 1, y: 0 }} exit = {{ opacity: 0, y: -5 }} className = "space-y-4" >
...
{/* ── ELECTRICITY BILL TAB ── */ }
{
  activeTab === 'electricity' && (
    <motion.div key="electricity" initial = {{ opacity: 0, y: 10 }
} animate = {{ opacity: 1, y: 0 }} exit = {{ opacity: 0, y: -5 }} className = "space-y-4" >
...
{
  activeTab === 'power' && (
    <motion.div key="power" initial = {{ opacity: 0, y: 10 }
} animate = {{ opacity: 1, y: 0 }} exit = {{ opacity: 0, y: -5 }} className = "space-y-4" >
                      ...
                      <p className={ cn('text-[8px] font-black uppercase tracking-widest', isDark ? 'text-white/30' : 'text-slate-400') }> Connection Steps </p>
{
  [
    ...
    },
    {
      ...
      desc: 'In AquaGrow, go to Smart Farm → Add Device. Select the device type, enter its serial number (on label), and assign it to a pond. The sensor will sync live readings in 2–3 minutes.',
    ...
    },
    {
      ...
      desc: 'Go to Water Monitor page. You should see ● LIVE badge on the health score widget. If readings show ---, press refresh or re-pair the device. Check signal strength — needs ≥ 60% for stable data.',
                ...
                < p className = { cn('text-[9px] font-medium leading-relaxed mb-2', isDark? 'text-white/40' : 'text-slate-500') } > { step.desc } </p>
                  < div className = "space-y-1" >
                  {
                    step.tips.map((tip, ti) => (
                      ...
                      <span className="text-[8px] mt-0.5" style = {{ color: step.color }} >›</span>
      ...
      </p>
{
  ...
  < p className = { cn('text-[8px] font-medium leading-snug', isDark? 'text-white/40' : 'text-slate-500') } >→ { item.fix } </p>
  </div>
  ))
}
</div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => setIsAddingDevice(false)}
                  className={cn('w-8 h-8 rounded-xl flex items-center justify-center', isDark ? 'bg-white/10' : 'bg-slate-100')}
                >
                  <X size={14} className={isDark ? 'text-white/60' : 'text-slate-600'} />
                </button>
              </div>
                ...
                {(['aerator', 'sensor', 'feeder', 'pump'] as IoTDevice['type'][]).map(type => {
                  ...
                  return (
                    <button
                      ...
                      className={cn('rounded-2xl p-3 flex flex-col items-center gap-1.5 border transition-all',
                        newDeviceType === type ? 'border-current shadow-sm' : (isDark ? 'bg-white/4 border-white/8' : 'bg-slate-50 border-slate-100')
                      )}
