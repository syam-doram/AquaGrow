  ...
  return (
    <div className="pb-32 bg-[#F8F9FE] min-h-screen">
      ...
      <div className="pt-24 px-6 py-6 sticky top-[60px] bg-[#F8F9FE]/80 backdrop-blur-xl z-10 border-b border-[#4A2C2A]/5">
        <div className="flex items-center bg-card rounded-2xl border border-card-border px-5 h-14 shadow-sm group focus-within:border-[#C78200]/30 transition-all">
          <Search size={20} className="text-ink/20 mr-3" />
          <input className="flex-1 bg-transparent outline-none text-sm font-black text-ink placeholder:text-ink/20" placeholder={t.searchGuides} />
        </div>
      </div>
      <div className="px-6 py-8 space-y-10">
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
          {[t.pondPrep, t.diseaseControl, t.harvesting, t.sustainability].map((tag, i) => (
            <button key={tag} className={cn(
              ...
              i === 0 ? "bg-[#C78200] text-white shadow-xl shadow-[#C78200]/20" : "bg-card border border-card-border text-ink/30 hover:text-[#C78200]"
            )}>
              {tag}
            </button>
          ))}
        ...
        <section className="space-y-6">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-lg font-black tracking-tighter text-ink">{t.pondPrep}</h3>
            <button className="text-[#C78200] text-[9px] font-black uppercase tracking-widest hover:text-ink transition-colors">{t.seeAll}</button>
          </div>
          <div className="flex overflow-x-auto gap-6 scrollbar-hide pb-4">
            {[1, 2].map(i => (
              <div key={i} className="min-w-[280px] flex flex-col gap-4 group cursor-pointer">
                <div className="relative aspect-video rounded-[2.5rem] overflow-hidden bg-card shadow-sm border border-card-border">
                  ...
                  <div className="absolute inset-0 bg-[#C78200]/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="w-12 h-12 bg-card rounded-2xl flex items-center justify-center shadow-2xl">
                      <PlayCircle size={24} className="text-[#C78200]" />
                    </div>
                  </div>
                </div>
                <div>
                  <p className="text-base font-black tracking-tight text-ink leading-tight group-hover:text-[#C78200] transition-colors">{t.soilTreatmentGuide}</p>
                  <div className="flex items-center gap-2 text-ink/30 text-[9px] font-black uppercase tracking-widest mt-2">
