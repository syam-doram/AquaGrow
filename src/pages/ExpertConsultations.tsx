...
export const ExpertConsultations = ({ user, t, onMenuClick }: { user: User, t: Translations, onMenuClick: () => void }) => {
  ...
  if (user.subscriptionStatus === 'free') {
    return (
      <div className="min-h-screen bg-[#FFFDF5] flex flex-col items-center justify-center p-10 text-center relative overflow-hidden">
        ...
        <p className="text-[#4A2C2A]/60 text-sm leading-relaxed mb-10 max-w-[240px] font-medium">{t.expertConsultations} is only available for AquaGrow Pro subscribers.</p>
        <button 
  ...
  return (
    <div className="pb-32 bg-[#F8F9FE] min-h-screen">
      <Header title={t.expertConsultations} showBack onMenuClick={onMenuClick} />
      <div className="pt-24 px-6 py-8 space-y-10">
        <div className="bg-[#C78200] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-xl shadow-[#C78200]/20">
          <div className="relative z-10">
            ...
            <h2 className="text-4xl font-black tracking-tighter leading-tight">Connect with Global <span className="italic font-serif">Bio-Experts</span></h2>
            <p className="text-white/60 text-sm mt-4 font-medium">Get instant advice on disease control, water quality, and yield optimization.</p>
          </div>
          <Sparkles size={100} strokeWidth={0.5} className="absolute -right-8 -bottom-8 text-white/5" />
        </div>

        <div className="space-y-6">
          <h3 className="text-lg font-black tracking-tighter text-[#4A2C2A] px-1">Available Experts</h3>
          <div className="space-y-6">
              ...
              <div key={expert.id} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-black/5 flex flex-col gap-6 group hover:border-[#C78200]/30 transition-all">
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[#4A2C2A]/30 text-[10px] font-black uppercase tracking-widest mt-1">{expert.experience} Experience</p>
                  </div>
                </div>
