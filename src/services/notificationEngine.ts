...
export function generateSmartAlerts(params: {
    ...
    if (records.length >= 3) {
      ...
      if (r0.ph != null && r1.ph != null && r2.ph != null) {
        ...
        if (phDrop > 0.3 && r0.ph >= 7.5 && r0.ph < 7.9) {
          alerts.push({
            id: uid(), category: 'pond_danger', priority: 'high',
            title: AT.trends.phDeclining.title(pond.name),
            body: AT.trends.phDeclining.body(r2.ph, r1.ph, r0.ph, records.length),
            action: AT.actions.logTreatment, actionRoute: `/pond/${pond.id}`,
            pondId: pond.id, pondName: pond.name,
            icon: '📉', timestamp: Date.now(), isRead: false,
          });
        }
        ...
        if (phDrop < -0.3 && r0.ph <= 8.5 && r0.ph > 8.1) {
          alerts.push({
            id: uid(), category: 'pond_danger', priority: 'medium',
            title: AT.trends.phRising.title(pond.name),
            body: AT.trends.phRising.body(r2.ph, r1.ph, r0.ph),
            action: AT.actions.logWater, actionRoute: `/logs/conditions`,
            pondId: pond.id, pondName: pond.name,
            icon: '📈', timestamp: Date.now(), isRead: false,
          });
      ...
      if (r0.do != null && r1.do != null && r2.do != null) {
        ...
        if (doDrop > 0.8 && r0.do >= 4.5 && r0.do < 5.5) {
          alerts.push({
            id: uid(), category: 'pond_danger', priority: 'high',
            title: AT.trends.doDropping.title(pond.name),
            body: AT.trends.doDropping.body(r2.do, r1.do, r0.do),
            action: AT.actions.viewPond, actionRoute: `/pond/${pond.id}`,
            pondId: pond.id, pondName: pond.name,
            icon: '⬇️', timestamp: Date.now(), isRead: false,
          });
      ...
      if (r0.ammonia != null && r1.ammonia != null && r2.ammonia != null) {
        ...
        if (ammoniaRise > 0.1 && r0.ammonia < 0.3) {
          alerts.push({
            id: uid(), category: 'pond_danger', priority: 'high',
            title: AT.trends.ammoniaRising.title(pond.name),
            body: AT.trends.ammoniaRising.body(r2.ammonia, r1.ammonia, r0.ammonia),
            action: AT.actions.logTreatment, actionRoute: `/logs/conditions`,
            pondId: pond.id, pondName: pond.name,
            icon: '⬆️', timestamp: Date.now(), isRead: false,
          });
  ...
  for (const pond of ponds.filter(p => p.status === 'active')) {
    ...
    if (riskFactors.length >= 2) {
      alerts.push({
        ...
        title: AT.trends.compoundRisk.title(riskFactors.length, pond.name),
        body: AT.trends.compoundRisk.body(riskFactors.join(' + ')),
        action: AT.actions.viewPond, actionRoute: `/pond/${pond.id}`,
        pondId: pond.id, pondName: pond.name,
  ...
  for (const pond of ponds.filter(p => p.status === 'active')) {
    ...
    if (mineralDaysAgo >= 5 && doc >= 10) {
      alerts.push({
        id: uid(), category: 'pond_danger', priority: 'medium',
        title: AT.trends.mineralGap.title(pond.name),
        body: AT.trends.mineralGap.body(mineralDaysAgo, pond.name, doc),
        action: AT.actions.logTreatment, actionRoute: `/medicine`,
        pondId: pond.id, pondName: pond.name,
        icon: '💊', timestamp: Date.now(), isRead: false,
      });
    }
    ...
    if (probioticDaysAgo >= 6 && doc >= 5) {
      alerts.push({
        id: uid(), category: 'disease', priority: 'medium',
        title: AT.trends.probioticGap.title(pond.name),
        body: AT.trends.probioticGap.body(probioticDaysAgo, pond.name),
        action: AT.actions.logTreatment, actionRoute: `/medicine`,
        pondId: pond.id, pondName: pond.name,
        icon: '🧫', timestamp: Date.now(), isRead: false,
      });
  ...
  for (const pond of ponds.filter(p => p.status === 'active')) {
    ...
    if (doc >= 50 && abw > 0 && abw < 20) {
      ...
      if (daysToHarvest <= 20 && daysToHarvest > 0) {
        alerts.push({
          id: uid(), category: 'harvest', priority: 'medium',
          title: AT.trends.harvestApproaching.title(pond.name),
          body: AT.trends.harvestApproaching.body(abw, doc, growthRate, harvestStr, daysToHarvest),
          action: AT.actions.viewMarket, actionRoute: `/finance`,
          pondId: pond.id, pondName: pond.name,
          icon: '🎯', timestamp: Date.now(), isRead: false,
        });
      }
  ...
  for (const pond of ponds.filter(p => p.status === 'active')) {
    ...
    if (totalFeed > 0 && biomass > 0) {
      ...
      if (fcr > 1.7 && fcr <= 2.2) {
        const waste = Math.round((fcr - benchmarkFCR) * biomass);
        alerts.push({
          id: uid(), category: 'feed', priority: 'high',
          title: AT.trends.fcrAboveBenchmark.title(fcr, wastePercent, pond.name),
          body: AT.trends.fcrAboveBenchmark.body(fcr, wastePercent, waste),
          action: AT.actions.reviewFeedLog, actionRoute: `/logs/feed`,
          pondId: pond.id, pondName: pond.name,
          icon: '📊', timestamp: Date.now(), isRead: false,
        });
